...
**附带收益——prompt 缓存命中**：压缩请求的输入为「完整原始上下文 + 追加在末尾的压缩指令」。完整上下文构成一个**稳定不变的前缀**（每次压缩与前一轮生成共享同一前缀），且压缩模型与主对话模型同配置——因此压缩调用可直接命中 LLM 服务端的上下文缓存（如 DeepSeek 的 prompt caching），前缀部分不再重复计费，压缩的增量成本仅等于末尾指令片段。
